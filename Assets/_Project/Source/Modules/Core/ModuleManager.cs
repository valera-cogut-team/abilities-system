...
namespace Core
{
    ...
    public class ModuleManager
    {
        ...
        {
            if (!_modules.TryGetValue(moduleName, out IModule m)) { Debug.WriteLine($"ModuleManager: Module '{moduleName}' not found"); return; }
        ...
        {
            if (!_enabledModules.TryGetValue(moduleName, out IModule m)) return;
        ...
        {
            _modules.TryGetValue(moduleName, out IModule m);
            return m;
        }
        ...
        {
            List<IModule> sorted = SortByDependencies(_modules.Values);
        ...
        {
            foreach (IModule m in _enabledModules.Values)
        ...
        private void Visit(IModule m, Dictionary<string, IModule> all, List<IModule> sorted, HashSet<string> visited, HashSet<string> visiting)
        {
            ...
            visiting.Add(m.Name);
            foreach (string d in m.Dependencies)
                if (all.TryGetValue(d, out IModule dep)) Visit(dep, all, sorted, visited, visiting);
