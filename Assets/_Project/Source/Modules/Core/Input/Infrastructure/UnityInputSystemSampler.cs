    ...
    public sealed class UnityInputSystemSampler : IInputSampler, IDisposable
    {
        ...
        public float SampleAxis(string axisName)
        {
            switch (axisName)
            {
                ...
                case "Mouse X":
                    return _look.ReadValue<Vector2>().x * InputConstants.MouseAxisScale;
                case "Mouse Y":
                    return _look.ReadValue<Vector2>().y * InputConstants.MouseAxisScale;
                default:
                    return 0f;
            }
        }
        ...
        public void SampleTouch(int index, out float x, out float y, out InputTouchPhase phase)
        {
            ...
            if (ts == null || index < 0 || index >= ts.touches.Count)
                return;
            TouchControl touch = ts.touches[index];
            Vector2 pos = touch.position.ReadValue();
            x = pos.x;
            y = pos.y;
            TouchPhase u = touch.phase.ReadValue();
        ...
        public bool SampleKey(int keyCode)
        {
            if (!KeyCodeInputMapper.TryMap(keyCode, out Key key))
                return false;
        ...
        public bool SampleKeyDown(int keyCode)
        {
            if (!KeyCodeInputMapper.TryMap(keyCode, out Key key))
                return false;
