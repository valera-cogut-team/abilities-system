...
namespace LifeCycle.Application
{
    public class LifeCycleService : ILifeCycleService
    {
        ...
        {
            IUpdateHandler[] buffer = null; int count = 0;
            ...
            try { for (int i = 0; i < count; i++) try { buffer[i].OnUpdate(deltaTime); } catch (Exception ex) { _logger.LogError($"Update handler error: {ex.Message}", ex); } }
            finally { ArrayPool<IUpdateHandler>.Shared.Return(buffer, clearArray: true); }
        }

        public void OnLateUpdate(float deltaTime)
        {
            ILateUpdateHandler[] buffer = null; int count = 0;
            ...
            try { for (int i = 0; i < count; i++) try { buffer[i].OnLateUpdate(deltaTime); } catch (Exception ex) { _logger.LogError($"LateUpdate handler error: {ex.Message}", ex); } }
            finally { ArrayPool<ILateUpdateHandler>.Shared.Return(buffer, clearArray: true); }
        }

        public void OnFixedUpdate(float fixedDeltaTime)
        {
            IFixedUpdateHandler[] buffer = null; int count = 0;
            ...
            try { for (int i = 0; i < count; i++) try { buffer[i].OnFixedUpdate(fixedDeltaTime); } catch (Exception ex) { _logger.LogError($"FixedUpdate handler error: {ex.Message}", ex); } }
