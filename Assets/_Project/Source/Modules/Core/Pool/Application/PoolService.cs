...
namespace Pool.Application
{
    public sealed class PoolService : IPoolService
    {
        ...
        public IObjectPool<T> CreatePool<T>(string id, Func<T> factory,
            int initialSize = PoolConstants.DefaultInitialSize, int maxSize = PoolConstants.DefaultMaxSize)
        ...
        public IObjectPool<T> GetPool<T>(string id) where T : class, IPoolable
        {
            lock (_lock)
            {
                if (_pools.TryGetValue(id, out object pool))
    ...
    {
        IObjectPool<T> CreatePool<T>(string id, Func<T> factory,
            int initialSize = PoolConstants.DefaultInitialSize, int maxSize = PoolConstants.DefaultMaxSize) where T : class, IPoolable;
    ...
    internal sealed class ObjectPool<T> : IObjectPool<T> where T : class, IPoolable
    {
        ...
        public ObjectPool(Func<T> factory, int initialSize, int maxSize)
        {
            ...
            int warm = Math.Max(0, initialSize);
